...
var DashboardHeatmaps = (function () {
  ...
  var _AppSalesHeatmap = function (element) {
    ...
    if ($(element).length > 0) {
      // Load data
      d3.csv("assets/demo_data/dashboard/app_sales_heatmap.csv", function (
        error,
        data
      ) {
        // Bind data
        ...
        x.domain([
          ...
          d3.time.hour.offset(new Date(data[data.length - 1].date), 1),
        ]);
        ...
        function resizeHeatmap() {
          ...
          (width =
            ...
            // Grid size
            (gridSize =
              width / new Date(data[data.length - 1].date).getHours()),
            // Height
            (height =
              (rowGap + gridSize) *
                d3.max(nest, function (d, i) {
