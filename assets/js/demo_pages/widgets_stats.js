...
var StatisticWidgets = (function () {
  ...
  var _areaChartWidget = function (element, chartHeight, color) {
    ...
    if (element) {
      ...
      d3.json("assets/demo_data/dashboard/monthly_sales.json", function (
        error,
        data
      ) {
        ...
        sidebarToggle &&
          sidebarToggle.addEventListener("click", messagesAreaResize);
  ...
  var _sparklinesWidget = function (
    ...
    if (element) {
      ...
      var sidebarToggle = document.querySelector(".sidebar-control");
      sidebarToggle &&
        sidebarToggle.addEventListener("click", resizeSparklines);
  ...
  var _progressIcon = function (
    ...
    if (element) {
      ...
      // Front path
      var front = svg.append("path").attr("class", "d3-progress-front").style({
        fill: foregroundColor,
        "fill-opacity": 1,
      });
  ...
  var _progressArcSingle = function (element, size) {
    ...
    if (element) {
      ...
      function update(v) {
        v = d3.format(".0f")(v);
        foreground.transition().duration(750).call(arcTween, v);
  ...
  var _progressArcMulti = function (element, size, goal) {
    ...
    if (element) {
      ...
      // Add SVG group
      var svg = container.attr("width", radius * 2).attr("height", radius + 20);
  ...
  var _roundedProgressMultiple = function (element, size) {
    ...
    if (element) {
      ...
      // Group
      var field = svg.selectAll("g").data(data).enter().append("g");
  ...
  var _pieWithProgress = function (element, size) {
    ...
    if (element) {
      ...
      // Colors
      var color = d3.scale.ordinal().range(["#EF5350", "#29b6f6", "#66BB6A"]);
  ...
  var _segmentedGauge = function (element, size, min, max, sliceQty) {
    ...
    if (element) {
      ...
      // Add SVG group
      var svg = container.attr("width", width).attr("height", height);
      ...
      var tickData = d3.range(slices).map(function () {
        return 1 / slices;
      });
