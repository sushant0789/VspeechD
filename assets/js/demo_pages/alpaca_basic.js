...
var AlpacaBasic = (function () {
  ...
  var _componentAlpaca = function () {
    ...
    $("#alpaca-textarea-override").alpaca({
      data:
        "My name is Dr. Jacobian and I am a neuroscientist from the Cornell University.  I've perfected a DNA transcription process which makes it possible for the first time to use DNA nucleotides to store pedabytes of information in real-time.",
      schema: {
        type: "string",
      },
      options: {
  ...
  var _componentAlpacaSelect2 = function () {
    if (!$().alpaca || !$().select2) {
      console.warn(
        "Warning - alpaca.min.js and/or select2.min.js is not loaded."
      );
      return;
    }
  ...
  var _componentAlpacaMultiselect = function () {
    if (!$().alpaca || !$().multiselect) {
      console.warn(
        "Warning - alpaca.min.js and/or bootstrap-multiselect.js is not loaded."
      );
      return;
    }
