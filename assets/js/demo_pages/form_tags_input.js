...
var TagInputs = (function () {
  ...
  var _componentTokenfield = function () {
    ...
    $("#create-token").on("click", function () {
      $("#create-token-field").tokenfield("createToken", {
        value: "new",
        label: "New token",
      });
    });
  ...
  var _componentTagsinput = function () {
    ...
    // Display values
    $(
      '.tags-input, [data-role="tagsinput"], .tagsinput-max-tags, .tagsinput-custom-tag-class'
    )
      .on("change", function (event) {
        ...
        $("pre.val > code", $container).html(
          $.isArray(val)
            ? JSON.stringify(val)
            : '"' + val.replace('"', '\\"') + '"'
        );
        $("pre.items > code", $container).html(
